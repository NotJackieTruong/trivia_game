...
export const replaceAll = (source = '', textReplace = '', textInstead = '') =>
  source.split(textReplace).join(textInstead);
export const removeHtmlTag = (source = '') => source.replace(/<\/?[^>]+(>|$)/g, '');
export const compareValue = (val1: any, val2: any) => equals(val1, val2);
export const removeChar = (source = '') => source.replace(/[^0-9]/g, '');
...
export const toFullWidth = (value: any) =>
  value + ''.replace(/[A-Za-z0-9]/g, s => String.fromCharCode(s.charCodeAt(0) + 0xfee0));
...
export const onHandleTagToArrayText = (source = '', char = '#'): Array<ResultHandleTagToArrayText> => {
  ...
  textSplit.forEach((text: string, i: number) => {
    const textData = { text, bold: false };
    if (text[0] === char) {
      textData.bold = true;
