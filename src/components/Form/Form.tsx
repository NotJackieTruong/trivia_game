...
const Child = ({ child, Inputs, setValue, trigger, i, getValues, errors }: ChildProps) => {
  const _onSubmitEditing = useCallback(() => {
    ...
    if (index < formatInput.length - 1) {
      formatInput[index + 1] ? formatInput[index + 1].focus() : formatInput[index].blur();
    } else {
...
const FormComponent = ({ register, errors, trigger, rules, setValue, getValues, children }: FormProps) => {
  ...
  const _renderItem = (child: JSX.Element, i: number) => {
    if (child.props.name) {
      return <Child key={i} {...{ child, i, setValue, getValues, errors, Inputs, trigger }} />;
    }
    return child;
  };
  return <>{(Array.isArray(children) ? [...children] : [children]).map(_renderItem)}</>;
};
