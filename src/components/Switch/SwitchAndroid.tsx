...
const SwitchComponent = ({
  ...
  const translateX = useInterpolate(progress, [0, 1], [OFF_POSITION, ON_POSITION], Extrapolate.CLAMP);
  const backgroundTrackColor = useInterpolateColor(progress, [0, 1], [OFF_TRACK_COLOR, ON_TRACK_COLOR]);
  const backgroundThumbColor = useInterpolateColor(progress, [0, 1], [OFF_COLOR, ON_COLOR]);

  // function
  // eslint-disable-next-line @typescript-eslint/naming-convention
  const _onToggle = useCallback(() => {
    if (typeof overwriteValue === 'boolean' && onCheckType(onToggle, 'function')) {
