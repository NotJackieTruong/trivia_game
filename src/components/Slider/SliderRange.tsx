...
import { sharedClamp, sharedMax, sharedSpring, sharedSub, useMin } from '#animated';
...
const SliderRangeComponent = ({
  ...
  const translateLeftX = useDerivedValue(() =>
    sharedClamp(translationLeftX.value, lowerBound - THUMB_SIZE, width - THUMB_SIZE)
  );

  const translationRightX = useSharedValue(0);
  const translateRightX = useDerivedValue(() =>
    sharedClamp(translationRightX.value, lowerBound - THUMB_SIZE, width - THUMB_SIZE)
  );

  const leftTrack = useMin(translateLeftX, translateRightX);
  const rightTrack = useDerivedValue(() =>
    sharedSub(width - THUMB_SIZE, sharedMax(translateLeftX.value, translateRightX.value))
  );

  // function
  const gestureHandlerThumbLeft = useAnimatedGestureHandler<PanGestureHandlerGestureEvent, { startX: number }>({
    onStart: (_, ctx) => {
  ...
  const gestureHandlerThumbRight = useAnimatedGestureHandler<PanGestureHandlerGestureEvent, { startX: number }>({
    onStart: (_, ctx) => {
  ...
  useEffect(() => {
    translationLeftX.value = sharedSpring(
      (Math.min(initialRange[0], initialRange[1]) / upperBound) * width - THUMB_SIZE
    );
    translationRightX.value = sharedSpring(
      (Math.max(initialRange[0], initialRange[1]) / upperBound) * width - THUMB_SIZE
    );
