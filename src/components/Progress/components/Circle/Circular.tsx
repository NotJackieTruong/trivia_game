...
import Animated, { Extrapolate, useAnimatedStyle, useDerivedValue } from 'react-native-reanimated';
...
export const CircularComponent = ({ progress, radius, fg, bg }: CircularProps) => {
  const progressAnimated = useDerivedValue(() => sharedTiming(progress));
  const firstRotate = useRadian(useInterpolate(progressAnimated, [0, 50], [0, 180], Extrapolate.CLAMP));
  const secondRotate = useRadian(useInterpolate(progressAnimated, [0, 100], [0, 360], Extrapolate.CLAMP));
  const secondOpacity = useInterpolate(progressAnimated, [0, 49.9999999, 50, 100], [0, 0, 1, 1], Extrapolate.CLAMP);
  ...
  const coverStyle = useMemo<ViewStyle>(
    () => enhance([baseStyle, styles.cover, { borderLeftColor: bg, borderTopColor: bg }]) as ViewStyle,
    [baseStyle, bg]
  );
  const indicatorStyle = useMemo<ViewStyle>(
    () => enhance([baseStyle, styles.indicator, { borderLeftColor: fg, borderTopColor: fg }]) as ViewStyle,
    [baseStyle, fg]
  );
