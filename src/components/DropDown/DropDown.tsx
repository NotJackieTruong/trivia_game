...
import Animated, { measure, runOnJS, runOnUI, useAnimatedRef, useAnimatedStyle } from 'react-native-reanimated';
...
const DropDownComponent = forwardRef((props: DropDownProps, _) => {
  ...
  const _onCheckRenderBottom = useCallback((): boolean => {
    const statusbarHeight = Platform.OS === 'android' ? StatusBar.currentHeight ?? 24 : inset.top;
  ...
  const wrapStyle = useMemo(
    () =>
      enhance([
        styles.wrapView,
        isVisible && (_onCheckRenderBottom() ? styles.wrapViewBottomOpened : styles.wrapViewTopOpened),
        style
