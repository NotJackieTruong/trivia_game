...
export const assignObjectColumn = (nColumns: number, index: number, targetObject: DataType): ObjectColumn => ({
  ...targetObject,
  ...{ column: index % nColumns }
});

export const assignObjectIndex = (index: number, targetObject: ObjectColumn): ObjectIndex => ({
  ...targetObject,
  ...{ index }
});

export const containMatchingUri = (dest: DataType[], source: DataType[]) =>
  source.filter(img => dest.findIndex(x => x.uri === img.uri) === -1);
