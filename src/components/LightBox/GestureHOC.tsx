...
export const GestureHOC = memo(({ image, source, onClose, backDropOpacity }: GestureHOCProps) => {
  ...
  const _gestureHandler = useAnimatedGestureHandler({
    onActive: (event, _) => {
      ...
      scale.value = interpolate(translateY.value, [-250, 0, 250], [0.8, 1, 0.8], Extrapolate.CLAMP);

      backDropOpacity.value = interpolate(translateY.value, [-100, 0, 100], [0.6, 1, 0.6], Extrapolate.CLAMP);
    },
    onEnd: () => {
  ...
  const top = useDerivedValue(
    () => translateY.value + interpolate(animatedProgress.value, [0, 1], [image.py, targetY.value], Extrapolate.CLAMP)
  );
  const left = useDerivedValue(
    () => translateX.value + interpolate(animatedProgress.value, [0, 1], [image.px, targetX.value], Extrapolate.CLAMP)
  );
  const width = useInterpolate(animatedProgress, [0, 1], [image.width, image.targetWidth], Extrapolate.CLAMP);
  const height = useInterpolate(animatedProgress, [0, 1], [image.height, image.targetHeight], Extrapolate.CLAMP);
  // reanimated style
  ...
  return (
    <PanGestureHandler onGestureEvent={_gestureHandler}>
      <Animated.View style={[imageStyle]}>
        <AnimatedFastImage style={[styles.image]} resizeMode="contain" source={source} />
      </Animated.View>
    </PanGestureHandler>
  );
}, isEqual);
