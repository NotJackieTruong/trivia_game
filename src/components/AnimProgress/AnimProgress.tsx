...
const AnimProcessComponent = forwardRef((props: AnimProcessProps, ref) => {
  ...
  const width1 = useDerivedValue(() => interpolate(widthPercent.value, [0, 0.12], [width, 0]));

  const translateX1 = useDerivedValue(() =>
    interpolate(widthPercent.value, [0, 0.2, 0.5], [-width, width * 1.75, width * 2])
  );

  const width2 = useDerivedValue(() => interpolate(widthPercent.value, [0, 0.5, 1], [width, width, 0]));

  const translateX2 = useDerivedValue(() =>
    interpolate(widthPercent.value, [0, 0.2, 0.4, 1], [-width, -width, width * 1.5, width * 2])
  );
  // reanimated style
  ...
  useEffect(() => {
    if (visible) {
      widthPercent.value = withRepeat(sharedTiming(1, { duration: 3500 }), -1, false);
    } else {
