...
const ImageRemoteComponent = (props: ImageRemoteProps) => {
  ...
  const container = useMemo(() => enhance([styles.container, containerStyle]), [containerStyle]);
  const imgStyle = useMemo<StyleProp<ImageStyle>>(() => enhance([styles.img, styleOverride]), [styleOverride]);
  ...
  return (
    <Block style={container}>
      <Animated.View style={[styles.viewOnLoad, imageOnloadStyle]}>{childrenOnload}</Animated.View>
      ...
      {error && <Animated.View style={[styles.viewError]}>{childrenError}</Animated.View>}
    </Block>
  );
};
