...
import { Animated as AnimatedRN, Easing as EasingRN, Platform } from 'react-native';

import { dimensions } from '#theme';

const dimensionUI = { width: dimensions.width, height: dimensions.height };
...
const isIphoneX = () =>
  ...
  !Platform.isPad &&
  !Platform.isTVOS &&
  (dimensionUI.height === 812 || dimensionUI.width === 812 || dimensionUI.height === 896 || dimensionUI.width === 896);

const onScroll = (contentOffset: { x?: Animated.Node<number>; y?: Animated.Node<number> }) =>
  Animated.event([
    {
      nativeEvent: {
        contentOffset
...
const { Value, timing, clockRunning, startClock, block, cond, stopClock } = Animated;

const runTiming = (clock: Animated.Clock, start: number, end: number, duration: number) => {
  const state = {
